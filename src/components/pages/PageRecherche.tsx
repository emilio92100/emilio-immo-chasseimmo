              ...
              <div key={c.id} onClick={() => onNavigate('fiche', c)}
                ...
                {(c.type_bien || c.budget_min || c.surface_min || equipements.length > 0) && (
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 10, paddingBottom: 10, borderBottom: '1px solid #f1f5f9' }}>
                    ...
                    {c.budget_min && <span style={{ fontSize: 12, fontWeight: 700, color: '#854d0e', background: '#fef9c3', padding: '3px 10px', borderRadius: 20, border: '1px solid #fde68a' }}>💰 {c.budget_max ? `${(c.budget_min/1000).toFixed(0)}–${(c.budget_max/1000).toFixed(0)}k€` : `min ${(c.budget_min/1000).toFixed(0)}k€`}</span>}
                    {c.surface_min && <span style={{ fontSize: 12, fontWeight: 600, color: '#64748b', background: '#f8fafc', padding: '3px 10px', borderRadius: 20, border: '1px solid #e3e8f0' }}>📐 {c.surface_max ? `${c.surface_min}–${c.surface_max}m²` : `min ${c.surface_min}m²`}</span>}
                    {c.nb_pieces_min && <span style={{ fontSize: 12, fontWeight: 600, color: '#64748b', background: '#f8fafc', padding: '3px 10px', borderRadius: 20, border: '1px solid #e3e8f0' }}>🚪 {c.nb_pieces_max ? `${c.nb_pieces_min}–${c.nb_pieces_max} pièces` : `min ${c.nb_pieces_min} pièces`}</span>}
